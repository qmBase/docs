...
4. qmBase requires a SSL connection. So you have to configure the IIS to use either a self signed certificate or use a regular certificate. The assigned url must not contain **qmbase**. You can use **qm-base** instead.
...
| https://3tgl2vf85cht.statuspage.io/           | n/a          | Status of send grid api                                              |                                                                     | No info about current status of send grid api. You do not know whether sending Email notifications is operational |
