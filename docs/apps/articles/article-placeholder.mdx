...
| Datenvergleich            | `{{#if (eq status "Open")}} <span class="badge badge-success">Open</span> {{else}} <span class="badge badge-secondary">Closed</span> {{/if}}` | Vergleich von Daten zur bedingten Anzeige.                                                                  |
