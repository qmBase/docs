...
TODO: Explain all available handlebarsjs helpers
...
| Dauer (Stunden → Einheit) | `{{formatHoursToDays task.durationHours}}`<br>`{{formatHoursToDays task.durationHours "hours"}}`                                                                        | Einheiten: `years, quarters, months, weeks, days, hours, minutes, seconds, milliseconds`. Default: `days`.  |
...
| Bild/URL                  | `{{{image product.imageUrl }}}`                                                                                                                                         | Achtung: **drei** geschweifte Klammern, damit HTML nicht ge-escaped wird.                                   |
...
Es ist hilfreich für einzelne Objekte wie Reklamationen oder Audits automatisch Berichte über das Objekt selbst und die damit verknüpften Informationen erstellen zu können. Diese Vorlagen können im Dokuemntenmanagement vordefiniert und dann in den einzelnen Apps verwendet werden (bisher nur Auditmanagement und Reklamationsmanagement). 

In der Ansicht zum Ausfüllen des Berichts gibt es unter dem Tab **Data** eine Übersicht aller zur Verfügung stehenden Daten, die abgefragt werden können. Im folgenden bieten wir einige Besipiele für gängige Informationen: 

### Abfrage von Informationen aus dem Objekt selbst:

| Platzhalter                                                      | Beschreibugn der Abfrage                                |
| -----------------------------------------------------------------| ------------------------------------------------------- |
|{{{id}}}                                                          | ID                                                      |
|{{{Title}}}                                                       | Titel                                                   |
...
Für die Vorlagen für Berichte zum Beispiel im Auditmanagement oder Reklamationsmanagement ist notwendig
...
<code>{{#each todos}}</code> bezeichnet demn Beginn der Schleife. Der Bestandteil *todos* gibt dabei an, dass die Schleife die verknüpften Maßnahmen abfragt.  <code>{{/each}}</code> bezeichnet das Ende der Schleife. 

Die Paramter innerhalb der Schleife fragen zu jeder Maßnahme die Informationen gemäß der Tabellenüberschrift ab. 
