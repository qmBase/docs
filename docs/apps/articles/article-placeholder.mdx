...
| Datenvergleich            | `{{#if (eq status "Open")}} Offen {{else}} Abgeschlossen {{/if}}`                               | Vergleich von Daten zur bedingten Anzeige.                                                                  |
...
## Platzhalter für Word-Dokumente (Docx)

Platzhalter für Word-Dokumente können in der Form `{{variable}}` verwendet werden. Die Variablen orientieren sich dabei an den oben beschriebenen Platzhaltern.

:::info[Keine vollständige Handlebar Template Unterstützung]
Momentan gibt es keine vollständige Handlebar Template Unterstützung für Word-Dokumente. Daher können nur einfache Platzhalter ersetzt werden. Schleifen und Bedingungen werden nicht unterstützt.
:::

Folgende Platzhalter stehen derzeit zur Verfügung:

| Platzhalter               | Beschreibung                       |
| ------------------------- | ---------------------------------- |
| `{{article.createdAt}}`   | Erstelldatum des Dokuments         |
| `{{article.createdBy}}`   | Ersteller des Dokuments            |
| `{{article.publishedAt}}` | Veröffentlichungsdatum der Version |
| `{{article.responsible}}` | Zuständige Person                  |
| `{{article.version}}`     | Aktuelle Version                   |
